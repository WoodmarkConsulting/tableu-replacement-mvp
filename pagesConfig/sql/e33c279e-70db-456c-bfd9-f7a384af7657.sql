...
-- Chart: Error Share by Drive Type (BarChartModule)
-- Data: Share of all errors attributed to each drive type (Antriebstyp).
-- Shape: BarChartModule rows { category: string, values: [percentage] }
--   percentage = SUM(max_has_occurrence) per drive type / SUM(max_has_occurrence) overall * 100
--   The bars sum to 100%.
...
FROM filtered f
CROSS JOIN totals t
GROUP BY f.Antriebstyp, t.total_errors
ORDER BY SUM(f.max_has_occurrence) DESC
