-- Battery chart:
-- Nach Batterie: count error records per battery, ranked by error count.
-- Parse all dashboard and connection filter values from the single JSON input.
...
-- Keep only named batteries and apply each optional filter when it is set.
SELECT Batterie AS category, array(CAST(COUNT(ecu_fault_nm) AS DOUBLE)) AS `values`
FROM westeurope_extollo_2026007fielddatadev75cd75.`2018001_xd_td_sofa_dev`.v_xd_diag_ecu_fault CROSS JOIN chart_input
WHERE Batterie IS NOT NULL AND trim(Batterie) <> ''
...
-- Return the 15 batteries with the highest number of error records.
GROUP BY Batterie ORDER BY COUNT(ecu_fault_nm) DESC LIMIT 15;
