-- Steuergeraet chart:
-- Steuergeraet: count errors records per ecu_nm, ranked by error count.
-- Parse the dashboard filters from the single JSON input before querying the source view.
WITH chart_input AS (
  SELECT from_json(CAST(:input AS STRING), 'STRUCT<ms_cd: STRING, ecu_fault_desc: STRING, engine_md_cd: STRING, trans_md_cd: STRING, md_cd: STRING, plant_letter_cd: STRING, ecus: STRING, prod_from: STRING, prod_to: STRING, diag_from: STRING, diag_to: STRING, selection_ecu_nm: STRING, selection_engine_series: STRING, selection_battery: STRING, selection_trans_md_cd_4: STRING, selection_country: STRING, selection_ms_nm: STRING>') AS params
)
SELECT ecu_nm AS category, array(CAST(COUNT(ecu_fault_nm) AS DOUBLE)) AS `values`
FROM westeurope_extollo_2026007fielddatadev75cd75.`2018001_xd_td_sofa_dev`.v_xd_diag_ecu_fault
CROSS JOIN chart_input
-- Exclude empty ECU names, apply optional filters, and return the top 15 categories.
WHERE ecu_nm IS NOT NULL AND trim(ecu_nm) <> ''
  AND (chart_input.params.ms_cd IS NULL OR array_contains(split(chart_input.params.ms_cd, ','), ms_cd))
  AND (chart_input.params.ecu_fault_desc IS NULL OR array_contains(split(chart_input.params.ecu_fault_desc, ','), ecu_fault_desc))
  AND (chart_input.params.engine_md_cd IS NULL OR array_contains(split(chart_input.params.engine_md_cd, ','), engine_md_cd))
  AND (chart_input.params.trans_md_cd IS NULL OR array_contains(split(chart_input.params.trans_md_cd, ','), trans_md_cd))
  AND (chart_input.params.md_cd IS NULL OR array_contains(split(chart_input.params.md_cd, ','), md_cd))
  AND (chart_input.params.plant_letter_cd IS NULL OR array_contains(split(chart_input.params.plant_letter_cd, ','), prod_plant_cd))
  AND (chart_input.params.ecus IS NULL OR array_contains(split(chart_input.params.ecus, ','), ecu_nm))
...
GROUP BY ecu_nm ORDER BY COUNT(ecu_fault_nm) DESC LIMIT 15;
