-- Motorbaureihe chart:
-- Motorbaureihe: count errors records per Motorbaureihe, ranked by error count.
...
-- Count error records per engine series after applying all optional filters.
SELECT Motorbaureihe AS category, array(CAST(COUNT(ecu_fault_nm) AS DOUBLE)) AS `values`
FROM westeurope_extollo_2026007fielddatadev75cd75.`2018001_xd_td_sofa_dev`.v_xd_diag_ecu_fault CROSS JOIN chart_input
WHERE Motorbaureihe IS NOT NULL AND trim(Motorbaureihe) <> ''
...
GROUP BY Motorbaureihe ORDER BY COUNT(ecu_fault_nm) DESC LIMIT 15;
