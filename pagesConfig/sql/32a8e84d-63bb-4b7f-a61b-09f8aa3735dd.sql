-- Transmission Model code chart:
-- Nach Transmission Model Code: count error records per trans_md_cd_4, ranked by error count.
-- trans_md_cd_4 is used directly; no silent derivation is applied.
...
-- Count error records per transmission code and return the top 15 categories.
SELECT trans_md_cd_4 AS category, array(CAST(COUNT(ecu_fault_nm) AS DOUBLE)) AS `values`
FROM westeurope_extollo_2026007fielddatadev75cd75.`2018001_xd_td_sofa_dev`.v_xd_diag_ecu_fault CROSS JOIN chart_input
WHERE trans_md_cd_4 IS NOT NULL AND trim(trans_md_cd_4) <> ''
...
GROUP BY trans_md_cd_4 ORDER BY COUNT(ecu_fault_nm) DESC LIMIT 15;
