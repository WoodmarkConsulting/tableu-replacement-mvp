-- Global KPI:
-- Anzahl Fahrzeuge: number of distinct vehicles in the filtered diagnosis data.
...
SELECT 'Anzahl Fahrzeug' AS label, CAST(COUNT(DISTINCT vehicle_id) AS DOUBLE) AS value
FROM westeurope_extollo_2026007fielddatadev75cd75.`2018001_xd_td_sofa_dev`.v_xd_diag_ecu_fault
CROSS JOIN chart_input
-- Keep only identified vehicles and apply every optional dashboard filter.
