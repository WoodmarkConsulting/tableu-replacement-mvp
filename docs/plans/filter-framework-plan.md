...
Add a shared filtering framework to the config-driven Next.js dashboard: global and tab-level filters, canonical dimensions mapped to per-chart SQL parameters, URL-shareable state, and applied-filter display (chips + print). Everything stays declarative (JSON + SQL); modules remain config-driven renderers.

## Confirmed decisions

- **State management:** Zustand store + URL-sync middleware, context-scoped per dashboard.
- **Mapping model:** Central filter _dimensions_ + per-chart `filterBindings` (dimensionId → sqlParamName).
...
- Pass `onSelectionChange` to selection-capable modules so selected rows enter the central framework selection flow.
...
- `QueryParameterValue` is scalar (`string | number | boolean | bigint | Date | null`) — **no array support**. Multi-select values must pass a comma-joined string; each affected SQL predicate must be rewritten from the current `COALESCE(:p,'')='' OR col=:p` form to `(:p IS NULL OR array_contains(split(:p, ','), col))`. This is a per-target SQL edit, not just a param widen. _(Confirm approach before Phase 6.)_
...
- Manual smoke test of `cudoTest`: global + tab filters change chart data, chart selection reaches the central callback, applied-filter chips render, URL round-trips (shareable link restores state), and print/export summary shows active filters.
